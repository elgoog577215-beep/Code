...
### Decision 3: 正式课堂优先 Docker，本机执行必须通过 GNU C++17 烟测

Docker 沙箱仍是正式课堂推荐执行方式；本机执行器保留，用于本地开发和缺 Docker 的演示环境。两者都使用 C++17 参数，但只有 Docker 能提供更强隔离。本机模式不能只检测 `g++ --version`，必须实际编译包含 `bits/stdc++.h` 的 C++17 代码，避免 macOS Apple clang 被误判为可用信息竞赛环境。
...
- [Risk] 本机 `g++` 可能是 Apple clang，缺少 `bits/stdc++.h`。  
  → Mitigation: 系统状态使用真实 GNU 竞赛模板烟测，不通过则标记 C++17 未就绪；正式课堂推荐 Docker `gcc:13`。

- [Risk] Docker 镜像首次拉取慢。  
  → Mitigation: 文档和状态提示说明 Docker 沙箱依赖；课堂部署前预拉取 `gcc:13`。
...
4. 更新判题服务和执行环境状态逻辑。
5. 更新学生端语言模板、文件名和编辑器模式。
6. 更新教师端环境展示文案。
7. 添加后端 C++17 判题测试和前端 typecheck。
8. 运行 OpenSpec strict validation、相关后端测试、前端 typecheck、`git diff --check`。
